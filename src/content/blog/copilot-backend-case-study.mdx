...
tags: ["GenAI", "System Design", "LLM Serving", "Infrastructure"]
---
...
Three groups, each with different needs:
...
This is worth stating clearly because a common over-engineering pattern is to add models at multiple stages (a model for context ranking, a model for quality scoring, a model for safety filtering on top of the generation model). Each additional model call adds latency. In a 100ms budget, there's room for exactly one model call, and it better be fast.
...
The 100ms budget is what makes this system interesting from a design perspective. Let's break down where every millisecond goes.
...
Prefix caching is the single most impactful optimization for code completion latency, and also the most complex to operate. It deserves its own section.
...
The tiered approach:
- **L1 (GPU HBM)**: hottest entries. The 50-100 most frequently accessed prefixes. Sub-millisecond load time.
- **L2 (Host DRAM)**: warm entries. Thousands of prefixes. 1-3ms load time over PCIe.
- **L3 (Redis cluster)**: cold entries across the fleet. 5-10ms load time over network.
...
This is not a minor detail. FIM increases acceptance rate on code completion by 15-25% compared to prefix-only generation, because the model can infer what the developer wants from the code structure on both sides.
...
**Mitigation**: the gateway tracks rolling p99 latency. If it exceeds 80ms (getting close to the 100ms budget), it starts rejecting new requests before they reach the model pool. The model pool drains its queue, latency recovers, and requests resume. This shed-load-early pattern is better than letting the queue build up and having every request timeout.
...
**Mitigation**: cache warming. When a repo becomes active (first request from that repo in a session), proactively compute and cache the common file-level prefixes in the background. This preemptive warming means the second and subsequent requests from that repo hit the cache.
...
**Mitigation**: A/B testing on context assembly strategies. Track acceptance rate per strategy variant. The context assembly pipeline is the fastest thing to iterate on (it's deterministic code, not model weights), so it's the right place to experiment.
...
**Mitigation**: monitor the speculation acceptance rate. If it drops below 50% for a particular language or code pattern, fall back to standard decode for those requests. The draft model's accuracy varies by language: it tends to be better on Python (uniform style) and worse on languages with complex syntax (Rust, Haskell).
